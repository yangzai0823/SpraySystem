...
int saveToFile(std::string fileName, const VWSCamera::ImageData &data) {
    ...
    for (int nPntIndex = 0; nPntIndex < nPointNum; ++nPntIndex) {
        outFile << pSrcValue[nPntIndex * 3 + 0] << " "
                << pSrcValue[nPntIndex * 3 + 1] << " "
                << pSrcValue[nPntIndex * 3 + 2] << std::endl;
    }
    outFile.close();
}

void testMove() {
    double start[6] = {-31.7623, 33.8082, -40.1508, 20.9592, -37.6706, -0.7618};
    double end[6] = {-27.0692, -55.7453, -43.5885, 105.0391, -122.7237, -53.6061};
...
void imgFunc_b(const VWSCamera::ImageData &data, void *pUser) {
    MainProcess::CameraCallbackData *cameraCallbackData =
        (MainProcess::CameraCallbackData *)pUser;
    ...
    static int i = 0;
    std::cout << ++i << "  " << data.RGB8PlanarImage.nTimeStamp << "      "
              << data.RGB8PlanarImage.nTimeStamp - q << std::endl;
...
void MainProcess::recevieData_Slot(QVariant data) {
    ...
    context_b.laserCouple1 = {plcdata.laser_bottom_head,
                              plcdata.laser_bottom_behind};
...
MainProcess::MainProcess() {
    ...
    qRegisterMetaType<std::vector<int32_t>>(
    ...
    connect(this, SIGNAL(sendPlcData_b(QVariant)), sm_bottom,
            SLOT(sendPlcData_Slot(QVariant)));
    connect(this, SIGNAL(sendImgData_b(QVariant)), sm_bottom,
            SLOT(sendImgData_Slot(QVariant)));
    connect(sm_bottom, SIGNAL(begintraj_Singal(QVariant, bool)), this,
    ...
    connect(this, SIGNAL(sendPlcData_u(QVariant)), sm_top,
            SLOT(sendPlcData_Slot(QVariant)));
    connect(this, SIGNAL(sendImgData_u(QVariant)), sm_top,
            SLOT(sendImgData_Slot(QVariant)));
...
int MainProcess::Start(QString &msg) {
    ...
    if (plc->getState() > 0) {
        connect(plc, SIGNAL(recevieData_Signal(QVariant)), this,
                SLOT(recevieData_Slot(QVariant)));
    } else {
        msg = "请先连接PLC";
        return -1;
    }
    ...
    if (mc->getState() > 0) {
        connect(mc, SIGNAL(getTrajParam_Signal(quint8)), this,
                SLOT(getTrajParam_Slot(quint8)));
        connect(mc, SIGNAL(sendToRBT_Signal(quint8)), this,
                SLOT(sendToRBT_Slot(quint8)));
    } else {
        msg = "请先连接运动控制器";
        return -1;
    }
...
ThreadSafeVector<vws::PlanTaskInfo> *
MainProcess::GetPlanTaskInfo(int upper_or_bottom) {
...
float MainProcess::getChainSpeed() const { return 80; }
...
float MainProcess::getChainUnits() const { return 1 / 0.4198727819755431f; }

Eigen::VectorXd MainProcess::getRobotWaitPose() const {
    Eigen::VectorXd val(6);
    val << 14.8 / 180.0 * M_PI, -55 / 180.0 * M_PI, -33.57 / 180.0 * M_PI,
        145.6 / 180.0 * M_PI, -105 / 180.0 * M_PI, -60.7 / 180.0 * M_PI;
...
void MainProcess::getTrajParam_Slot(quint8 axisNum) {
    ...
    if (mcQueue.count() > 0) {
        ...
        QString strlog = "MC(" + QString::number(axisNum) + ")，发送信息" +
                         QString::number(zeropoint);
        CLog::getInstance()->log(strlog);
        MCOperator::TrajParamData trajpara;
        memset(&trajpara, 0, sizeof(MCOperator::TrajParamData));
        trajpara.zeropoint = param[0];
        trajpara.offset = param[1];
        trajpara.dpos_start = param[2];
        trajpara.dpos_mid = param[3];
        trajpara.dpos_stop = param[4];
        trajpara.axisNum = axisNum;
        mc->sendTrajParam(trajpara);
        // mc->sendTrajParam(zeropoint, offset, axisNum);

        mcRequest = false;
        axisNum = axisNum;
    } else {
        CLog::getInstance()->log("MC(" + QString::number(axisNum) +
                                 ")，请求数据，前队列为空无数据发送");
...
void MainProcess::sendToRBT_Slot(quint8 axisNum) {
    ...
    nRbt++;
    CLog::getInstance()->log("RBT(" + QString::number(axisNum) +
                             ")， 机器人发送信息");
    ...
    if (ret > 0) {
        CLog::getInstance()->log("RBT(" + QString::number(axisNum) +
                                 ")， 机器人发送任务成功(" + QString::number(nRbt) +
                                 "), 点数： " + QString::number(rbtparam.size()) +
                                 ", 队列剩余： " +
                                 QString::number(trajQueue.size()));
    } else {
        if (nFail < 1) {
...
void MainProcess::SetRobotTaskInfo(std::vector<float> mc_data,
...
void MainProcess::begintraj_Slot(QVariant vdata, bool up_or_bttom) {
    ...
    if (up_or_bttom) {
        ...
        CLog::getInstance()->log(
            "队列数目 " + QString::number(safeQPlanTaskInfoTop.getVector().size()));
    } else {
        ...
        CLog::getInstance()->log(
            "队列数目 " + QString::number(safeQPlanTaskInfoBottom.getVector().size()));
    }
