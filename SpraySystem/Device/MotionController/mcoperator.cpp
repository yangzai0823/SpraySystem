...
MCOperator::MCOperator(std::shared_ptr<MotionController> mc)
{
    ...
    data_slave.mcErrs.resize(3);
    data_slave.mcErrs.clear();
}
...
int MCOperator::init()
{
    ...
    connect(this, SIGNAL(connect_master_Signal(QString, int)), master_socket,
            SLOT(connect_Slot(QString, int)),
            Qt::ConnectionType::QueuedConnection);
    connect(this, SIGNAL(connect_slave_Signal(QString, int)), slave_socket,
            SLOT(connect_Slot(QString, int)),
            Qt::ConnectionType::QueuedConnection);
...
int MCOperator::start()
{
    ...
    while (!stop) {
      ...
      CLog::getInstance()->log("应答MC, order: " +
                               QString::number(data_slave.order));
      // QString current_date_time1 =
      // QDateTime::currentDateTime().toString(Qt::ISODateWithMs);
      // std::cout<<"应答: "<<current_date_time1.toStdString()<<std::endl;
      slave_socket->send(byArr);
      if (data_slave.order == 33) {
        CLog::getInstance()->log("Receive from MC, 执行发送轨迹规划参数");
        emit getTrajParam_Signal(data_slave.axis_num);
      } else if (data_slave.order == 35) {
        CLog::getInstance()->log("Receive from MC, 执行发送轨迹给机器人");
        emit sendToRBT_Signal(data_slave.axis_num);
      } else if (data_slave.order == 36) {
        CLog::getInstance()->log("Receive from MC, 运动控制器报警");
        emit mcWarning_Signal(data_slave.axis_num, data_slave.mcErrs);
      }
    } });
    t_slave.detach();
    return 1;
...
int MCOperator::getState() { return 1; }
...
void MCOperator::move(float step, uint8_t axisNum)
{
    trySendData(3, step, 0, data_master.b_move, axisNum);
}
