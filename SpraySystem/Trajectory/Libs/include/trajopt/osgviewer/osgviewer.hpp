...
class TRAJOPT_API OSGViewer : public OpenRAVE::ViewerBase {
 public:
  static boost::shared_ptr<OSGViewer> GetOrCreate(
  ...
  void TakeScreenshot();
  bool GetLastScreenshot(std::vector<unsigned char>& image,
                         unsigned int& height, unsigned int& width);

  // return false if you want to disable the default TrackballManipulator
  // handling
  ...
  void AddKeyCallback(int key, const KeyCallback& cb,
                      const std::string& help = "");
  void PrintHelp();

  OpenRAVE::GraphHandlePtr drawarrow(const RaveVectorf& p1,
                                     const RaveVectorf& p2, float fwidth,
                                     const RaveVectorf& color);
  OpenRAVE::GraphHandlePtr drawtrimesh(const float* ppoints, int stride,
                                       const int* pIndices, int numTriangles,
                                       const RaveVectorf& color);
  //  OpenRAVE::GraphHandlePtr drawtrimesh (const float *ppoints, int stride,
  //  const int *pIndices, int numTriangles, const boost::multi_array< float, 2
  //  > &colors);
  OpenRAVE::GraphHandlePtr plot3(const float* ppoints, int numPoints,
                                 int stride, float fPointSize,
                                 const OpenRAVE::RaveVector<float>& color,
                                 int drawstyle = 0);
  OpenRAVE::GraphHandlePtr plot3(const float* ppoints, int numPoints,
                                 int stride, float pointsize,
                                 const float* colors, int drawstyle = 0,
                                 bool bhasalpha = false);
  OpenRAVE::GraphHandlePtr drawlinelist(const float* ppoints, int numPoints,
                                        int stride, float fwidth,
                                        const RaveVectorf& color);
  OpenRAVE::GraphHandlePtr drawlinestrip(const float* ppoints, int numPoints,
                                         int stride, float fwidth,
                                         const RaveVectorf& color);
  OpenRAVE::GraphHandlePtr _drawlines(osg::PrimitiveSet::Mode mode,
                                      const float* ppoints, int numPoints,
                                      int stride, float fwidth,
                                      const RaveVectorf& color);
  OpenRAVE::GraphHandlePtr drawtext(const std::string& text, float x, float y,
                                    float fontsize,
...
void TRAJOPT_API SetColor(OpenRAVE::GraphHandlePtr handle,
