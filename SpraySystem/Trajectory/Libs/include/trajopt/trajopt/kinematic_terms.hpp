...
struct ZAxisPoseErrCalculator : public VectorOfVector {
  OR::Vector zdir_;
  ConfigurationPtr rad_;
  OR::RobotBase::ManipulatorPtr manip_;
  // OR::KinBody::LinkPtr link_;
  ZAxisPoseErrCalculator(const OR::Vector& zdir, ConfigurationPtr rad,
                         OR::RobotBase::ManipulatorPtr manip,
                         OR::KinBody::LinkPtr link)
      : zdir_(zdir), rad_(rad), manip_(manip) {
    zdir_ = zdir_.normalize4();
  }
  VectorXd operator()(const VectorXd& dof_vals) const;
};

struct CartXYZErrCalculator : public VectorOfVector {
  OR::Vector xyz_;
  ConfigurationPtr rad_;
  OR::RobotBase::ManipulatorPtr manip_;
  // OR::KinBody::LinkPtr link_;
  CartXYZErrCalculator(const OR::Vector& xyz, ConfigurationPtr rad,
                       OR::RobotBase::ManipulatorPtr manip,
                       OR::KinBody::LinkPtr link)
      : xyz_(xyz), rad_(rad), manip_(manip) {}
  VectorXd operator()(const VectorXd& dof_vals) const;
};

struct CartXYZErrCalculatorPlotter : public Plotter {
  boost::shared_ptr<void>
      m_calc;  // actually points to a CartPoseErrCalculator = CartPoseCost::f_
  VarVector m_vars;
  CartXYZErrCalculatorPlotter(boost::shared_ptr<void> calc,
                              const VarVector& vars)
      : m_calc(calc), m_vars(vars) {}
  void Plot(const DblVec& x, OR::EnvironmentBase& env,
            std::vector<OR::GraphHandlePtr>& handles);
};

struct CartToolPoseErrCalculator : public VectorOfVector {
  OR::Transform pose_inv_;
  ConfigurationPtr rad_;
  OR::RobotBase::ManipulatorPtr manip_;
  // OR::KinBody::LinkPtr link_;
  CartToolPoseErrCalculator(const OR::Transform& pose, ConfigurationPtr rad,
                            OR::RobotBase::ManipulatorPtr manip,
                            OR::KinBody::LinkPtr link)
      : pose_inv_(pose.inverse()), rad_(rad), manip_(manip) {}
  VectorXd operator()(const VectorXd& dof_vals) const;
};

struct CartToolPoseErrPlotter : public Plotter {
  boost::shared_ptr<void>
      m_calc;  // actually points to a CartPoseErrCalculator = CartPoseCost::f_
  VarVector m_vars;
  CartToolPoseErrPlotter(boost::shared_ptr<void> calc, const VarVector& vars)
      : m_calc(calc), m_vars(vars) {}
  void Plot(const DblVec& x, OR::EnvironmentBase& env,
            std::vector<OR::GraphHandlePtr>& handles);
};
...
struct CartPoseErrCalculator : public VectorOfVector {
  ...
  OR::KinBody::LinkPtr link_;
  CartPoseErrCalculator(const OR::Transform& pose, ConfigurationPtr manip,
                        OR::KinBody::LinkPtr link)
...
struct CartPoseErrorPlotter : public Plotter {
  boost::shared_ptr<void>
      m_calc;  // actually points to a CartPoseErrCalculator = CartPoseCost::f_
  VarVector m_vars;
  CartPoseErrorPlotter(boost::shared_ptr<void> calc, const VarVector& vars)
      : m_calc(calc), m_vars(vars) {}
  void Plot(const DblVec& x, OR::EnvironmentBase& env,
            std::vector<OR::GraphHandlePtr>& handles);
};
...
struct CartVelJacCalculator : MatrixOfVector {
  ...
  double limit_;
  CartVelJacCalculator(ConfigurationPtr manip, KinBody::LinkPtr link,
                       double limit)
